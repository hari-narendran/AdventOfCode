...
class DirectoryTree {
  ...
  findDirectory(dirName) {
    return this.dirTree.find((dir) => dir.name == dirName);
  }
}
...
let changeDir = (arg) => {
  switch (arg) {
    ...
    default:
      ...
      dir.setParents(currentPath.slice(1, currentPath.length));
...
let addSizeToParents = (parents, subDirSize) => {
  parents.forEach((parent) => {
    if (parent) {
      let parentDir = directoryTree.findDirectory(parent);
      addSizeToParent(parent, subDirSize);
    }
  });
};
...
let updateDirSize = (dirTree) => {
  dirTree.forEach((dir) => {
    ...
    if (dir.parent) {
      ...
      addSizeToParents(dir.parents, subDirSize);
    }
  });
};
...
let findParents_ = (dir) => {
  let parents = [];
  if (dir.parent) {
    parents.push(dir.parent);
    //console.log('Find Parent')
    //console.log(directoryTree.findDirectory(dir.parent));
    let parentDir = directoryTree.findDirectory(dir.parent);
    findParents(parentDir);
    return parents;
  } else {
    return [];
  }
